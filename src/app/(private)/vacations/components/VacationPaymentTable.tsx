        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right w-32">13º Salário</TableHead>
          ...
          <TableBody>
            {items.map((v) => (
              <TableRow key={v.id} className="hover:bg-gray-50">
                <TableCell className="font-mono text-sm">{String(v.employeeId).padStart(3, "0")}</TableCell>
                ...
                <TableCell className="text-right font-medium text-sm">
                  R$ {(v.thirteenthAdvance ? v.baseSalary * 0.5 : 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                </TableCell>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-24">Mês</TableHead>
            <TableHead className="w-20">Dias</TableHead>
            <TableHead className="text-right w-28">Férias</TableHead>
            <TableHead className="text-right w-28">1/3 Férias</TableHead>
            <TableHead className="text-right w-28">Abono</TableHead>
            <TableHead className="text-right w-28">1/3 Abono</TableHead>
            <TableHead className="text-right w-32">Total Férias</TableHead>
            <TableHead className="text-right w-32">Total Abono</TableHead>
            <TableHead className="text-right w-32 font-bold">Total Geral</TableHead>
            <TableHead className="w-24">Status</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {items.map((v) => {
            const totalFerias = (v.vacationValue || 0) + (v.onethirdValue || 0);
            const totalAbono  = (v.abonoValue || 0) + (v.abonoOnethirdValue || 0);
            ...
            return (
              <TableRow key={v.id} className="hover:bg-gray-50">
                ...
                <TableCell className="text-sm">{months[getPaymentMonth(v) - 1]}</TableCell>
                <TableCell className="text-sm">{v.vacationDays}</TableCell>
                <TableCell className="text-right text-sm">R$ {(v.vacationValue || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</TableCell>
                <TableCell className="text-right text-sm">R$ {(v.onethirdValue || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</TableCell>
                <TableCell className="text-right text-sm">R$ {(v.abonoValue || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</TableCell>
                <TableCell className="text-right text-sm">R$ {(v.abonoOnethirdValue || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</TableCell>
