// FILE: src/app/(private)/overtimes/components/OvertimeSectorAnalysisTable.tsx
"use client";
...
export default function OvertimeSectorAnalysisTable({ rows }: Props) {
    return (
            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        ...
                        <TableHead className="text-center min-w-16 sm:min-w-20">Feriados</TableHead>
                        <TableHead className="text-center min-w-16 sm:min-w-20">Noturno</TableHead>
                        <TableHead className="text-right min-w-20">DSR</TableHead>
                        <TableHead className="text-right min-w-24">DSR Not.</TableHead>
                ...
                <TableBody>
                    {rows.map((r) => (
                        <TableRow key={r.id}>
                            ...
                            <TableCell className="text-center text-sm">{r.holidayHours ?? 0}h</TableCell>
                            <TableCell className="text-center text-sm">{r.nightShiftHours ?? 0}h</TableCell>
                            <TableCell className="text-right text-sm">
                                R$ {(r.dsrValue ?? 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                            </TableCell>
                            <TableCell className="text-right text-sm">
                                R$ {(r.dsrNightValue ?? 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                            </TableCell>
                            ...
                            <TableCell className="text-right">
                                <div className={`text-sm font-medium ${vClass(Math.abs(r.variancePercentage))}`}>
